...
namespace GrooveCaster
{
    public static class Database
    {
        ...
        private static String m_StatsDbConnectionString;
        ...
        public static IDbConnection GetStatsConnection()
        {
            return m_StatsDbConnectionString.OpenDbConnection();
        }
        ...
        {
            m_StatsDbConnectionString = "gcaster.stats.sqlite";

            if (!Application.SelfHosted)
                m_StatsDbConnectionString = Path.Combine(AppDomain.CurrentDomain.GetData("DataDirectory").ToString(), "gcaster.stats.sqlite");
            
            using (var s_Db = GetStatsConnection())
            {
                if (!s_Db.TableExists<StatisticsUnit>())
                {
                    s_Db.CreateTable<StatisticsUnit>();
                }
            }
        }
